...
export default class Home extends React.Component {
    ...
    async componentDidMount() {
        await this.setState({error: null, auctions: [], offset: 0})
        ...
        fetch(this.props.session.ip+'/', requestOptions)
            ...
            .catch(error => {
                ...
                if(error.message === "Failed to fetch") {
                    this.setState({error: "Cannot reach auction-dw server. Is it down? ("+error.name+": "+error.message+")"})
                } else {
    ...
    update_jumbo(make_smaller) { // small = true, big = false
        if(make_smaller) {
            $('div.jumbotron.home-jumbo').addClass("home-small-jumbo")
        } else {
            $('div.jumbotron.home-jumbo').removeClass("home-small-jumbo")
        }
    }
                    ...
                    {this.state.error ? (
                        ...
                        (<div className="profile-content">
                            ...
                            <div className="ww1-header">
                                ...
                                <h1 className="display-4">Historical Image Auctions</h1>
    ...
    load_more = async (starting_offset) => {
        let requestOptions = {
            ...
            body: JSON.stringify({
                "amount": this.state.base_offset,"offset": starting_offset
            })
        };
        console.log(requestOptions)
        await fetch(this.props.session.ip+'/auctions/get_featured_auctions', requestOptions)
            .then(async response => {
                const data = await response.json();
                // check for error response
                if (!response.ok) {
    ...
    async get_auctions(refresh) {
        let requestOptions = {
            ...
            headers: {'session_key': this.props.session.key, 'Content-Type': 'application/json' },
            body: JSON.stringify({
                "amount": this.state.base_offset,"offset": 0
            })
        };
        fetch(this.props.session.ip+'/auctions/get_featured_auctions', requestOptions)
            .then(async response => {
                const data = await response.json();
                // check for error response
                if (!response.ok) {
